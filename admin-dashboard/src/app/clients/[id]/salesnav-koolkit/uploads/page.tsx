...
import {
  getClientById,
  getSalesNavKoolKitUploads,
  Client,
  SalesNavKoolKitUpload,
} from "@/app/actions";

export default function SalesNavKoolKitPastUploadsPage() {
  const params = useParams();
  const clientId = params.id as string;

  const [client, setClient] = useState<Client | null>(null);
  const [uploads, setUploads] = useState<SalesNavKoolKitUpload[]>([]);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const [clientData, uploadsData] = await Promise.all([
        ...
        getSalesNavKoolKitUploads(clientId),
      ]);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                {client.company_name} - Previous SalesNav KoolKit CSV uploads
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-2xl mx-auto">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                View and manage previous SalesNav KoolKit CSV uploads
              </CardDescription>
            </CardHeader>
            <CardContent>
              {uploads.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <Link
                    href={`/clients/${clientId}/salesnav-koolkit/upload`}
